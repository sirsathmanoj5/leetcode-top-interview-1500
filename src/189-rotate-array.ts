function rotateArrayByK(nums: number[], k: number): void {
    const N = nums.length;
    k=k%N;
    let i=0;
...
function rotateArrayByKUsingSlice(nums: number[], k: number):void {
    const N = nums.length;
    k = k%N;

    if(k!==0) {
        let temp: number[] = nums.slice(-k).concat(nums.slice(0,-k))
...
function reverse(nums: number[], first: number, last: number): void {
...
function rotateArrayByKInPlace(nums: number[], k: number): number[] {
    const N=nums.length;
    k=k%N;
