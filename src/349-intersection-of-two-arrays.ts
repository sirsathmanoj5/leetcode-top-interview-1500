...
function intersectionOfTwoArray(arr1: number[], arr2: number[]): number[] {
    const ans = [];
    // sort two arrays
    arr1.sort((a,b) => a-b);
    arr2.sort((a,b) => a-b);
    ...
    const setData: Set<number> = new Set<number>();
...
// using map
function intersectionOfTwoArrayUsingMap(arr1: number[], arr2: number[]): number[] {
    //initialize the map
    const seenMap: Map<number, Number> = new Map<number, number>();
    const result: number[] = [];
    for(let ele of arr1) {
        seenMap.set(ele, 1);
    }
